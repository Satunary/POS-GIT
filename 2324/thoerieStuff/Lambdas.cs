...
class LambdaTest
{
    ...
    public static void ForEach(decimal[] values, Func<decimal, decimal> action)
    {
        foreach (decimal value in values)
        {
            action(value);
    ...
    /// <returns></returns>
    public static decimal ArithmeticOperation(decimal x, decimal y,Func<decimal,decimal,decimal> operation,Func<string,string> logFunction)
    {
        try
        {
            return operation(x, y);
    ...
    public static void RunCommand(Func<int> command)
    {
        command();
    }
